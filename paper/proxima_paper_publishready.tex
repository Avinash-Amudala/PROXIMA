...
A/B testing has become ubiquitous in technology companies \cite{kohavi2009controlled,kohavi2013online}. The challenge of long measurement windows has led to extensive research on proxy metrics and surrogate endpoints \cite{deng2016continuous}. Recent work has focused on improving experiment sensitivity \cite{deng2013improving} and reducing time-to-decision through variance reduction techniques.

\subsection{Early Stopping and Sequential Testing}

Sequential testing methods allow experimenters to monitor results continuously and stop early when sufficient evidence accumulates \cite{johari2017peeking}. Group sequential designs \cite{jennison1999group} and always-valid inference \cite{howard2021time} provide statistical guarantees for early stopping. However, these methods assume the metric being monitored is the true outcome of interest—they do not address whether an \textit{early proxy metric} reliably predicts a \textit{different long-term outcome}. PROXIMA complements sequential testing by validating which proxies are safe to use for early decisions.

\subsection{Metric Gaming and Goodhart's Law}

Goodhart's Law states that ``when a measure becomes a target, it ceases to be a good measure'' \cite{goodhart1984problems}. In online experimentation, this manifests as metric gaming—optimizing for proxy metrics in ways that harm long-term outcomes \cite{liu2019understanding}. For example, clickbait headlines may increase click-through rate while decreasing user satisfaction. PROXIMA's fragility detection helps identify proxies vulnerable to gaming by detecting segment-level divergence between proxy and long-term effects.

\subsection{Proxy Metrics and Surrogate Outcomes}

Prior work has studied correlation-based proxy selection \cite{deng2013improving} and sensitivity analysis \cite{deng2017statistical}. The surrogate paradox in causal inference \cite{chen2007criteria} shows that even strongly correlated surrogates can lead to incorrect causal conclusions. Prentice criteria \cite{prentice1989surrogate} formalize conditions for valid surrogates in clinical trials. However, these approaches focus on global correlation and miss segment-level failures that PROXIMA explicitly detects.

\subsection{Simpson's Paradox}

Simpson's Paradox describes situations where aggregate trends reverse when data is partitioned \cite{simpson1951interpretation,pearl2014understanding}. While well-studied in statistics, automated detection in A/B testing contexts remains underexplored. PROXIMA operationalizes Simpson's Paradox detection through segment-level fragility analysis.

\subsection{Treatment Effect Heterogeneity}

Recent work on heterogeneous treatment effects \cite{athey2016recursive,kunzel2019metalearners} provides methods for estimating segment-level effects. Causal forests \cite{wager2018estimation} enable personalized treatment effect estimation. However, these methods focus on estimating heterogeneity in a single outcome, not on validating whether proxy metrics remain reliable across segments—the core contribution of PROXIMA.
...
Metrics (at the experiment level):
\begin{itemize}
    \item \textbf{Win rate}: fraction of experiments where the proxy decision (ship / no-ship) matches the oracle decision.
    \item \textbf{False positive rate (FPR)}: $\mathrm{FP} / \max(1,\mathrm{Shipped}_{proxy})$ (ship when oracle would not).
    \item \textbf{False negative rate (FNR)}: $\mathrm{FN} / \max(1,\mathrm{Shipped}_{oracle})$ (do not ship when oracle would).
    \item \textbf{Regret}: average long-term ATE gap between oracle and proxy decision outcomes.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Split}: 85\% treatment / 15\% control (11.88M / 2.10M rows)
\end{itemize}
...
\textbf{Proxy} & \textbf{Win} & \textbf{FP} & \textbf{FN} & \textbf{Reg.} & \textbf{Ships} \\
\midrule
early\_starts & \textbf{0.97} & 0.03 & 0.00 & 0.0023 & 30 \\
early\_ctr & \textbf{0.97} & 0.03 & 0.00 & 0.0023 & 30 \\
early\_watch & 0.96 & 0.04 & 0.14 & -0.0053 & 26 \\
Oracle & 1.00 & 0.00 & 0.00 & 0.0000 & 29 \\
rebuffer\_rate & 0.00 & 0.00 & 1.00 & -0.1147 & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item One false positive out of 30 shipped decisions (FPR $\approx$ 3.3\%) - acceptable error rate
\end{itemize}
...
PROXIMA is designed to improve decision-making in online experiments, but we acknowledge potential risks:

\begin{enumerate}
    \item \textbf{Metric manipulation}: If practitioners know which proxies are scored as ``reliable,'' they may optimize for those proxies at the expense of unmeasured outcomes. Organizations should maintain a diverse set of guardrail metrics and periodically re-validate proxy reliability.

    \item \textbf{Bias amplification}: Proxy metrics may encode historical biases. For example, engagement metrics trained on past user behavior may perpetuate filter bubbles or discriminatory recommendations. We recommend auditing proxy-outcome relationships across demographic groups.

    \item \textbf{Short-termism}: Over-reliance on proxy metrics—even validated ones—may incentivize short-term optimization at the expense of long-term user welfare. PROXIMA should complement, not replace, periodic long-term outcome measurement.

    \item \textbf{Transparency}: Organizations using PROXIMA should document which proxies are used for decisions and their reliability scores, enabling accountability and external auditing.
\end{enumerate}

We encourage practitioners to use PROXIMA as one input to decision-making, not as an automated oracle, and to maintain human oversight of experiment outcomes.
...
\begin{enumerate}
    ...
    \item \textbf{Fairness-aware validation}: Extend fragility detection to demographic subgroups
\end{enumerate}
...
\textit{The Annals of Statistics}, 49(2), 1055-1080.

\bibitem{goodhart1984problems}
Goodhart, C. A. E. (1984).
Problems of monetary management: the UK experience.
\textit{Monetary Theory and Practice}, 91-121.

\bibitem{liu2019understanding}
Liu, Y., Radlinski, F., Craswell, N., \& Shokouhi, M. (2019).
Understanding and mitigating position bias in click models.
\textit{WSDM}, 241-249.

\bibitem{chen2007criteria}
Chen, H., Geng, Z., \& Jia, J. (2007).
Criteria for surrogate end points.
\textit{Journal of the Royal Statistical Society: Series B}, 69(5), 919-932.

\bibitem{prentice1989surrogate}
Prentice, R. L. (1989).
Surrogate endpoints in clinical trials: definition and operational criteria.
\textit{Statistics in Medicine}, 8(4), 431-440.
